...
def main(argv):
    ...
    agroup.add_argument('-l', '--list',
        action='store_true',
        help='list the contacts in the file'
    )
    agroup.add_argument('-s', '--stats',
        action='store_true',
        help='display statistics of entries'
    )
    aparser.add_argument('-y', '--yaml',
        action='store_true',
        help='output statistics as YAML instead of JSON'
    )
    aparser.add_argument('--no-output',
        action='store_true',
        help='do not write output when using --list or --stats, use - as the output file',
    )

    aparser.add_argument('--sort-contact',
        action='store_true',
        help='sort list output by contact instead of number'
    )

    aparser.add_argument('-d', '--filter-contact-expr',
        action='append', metavar='EXPR',
        help='add contact expression to filter'
    )
    aparser.add_argument('-e', '--filter-number-expr',
        action='append', metavar='EXPR',
        help='add number expression to filter'
    )
    aparser.add_argument('-f', '--filter-contact',
        action='append', metavar='NAME',
        help='add contact to filter'
    )
    aparser.add_argument('-g', '--filter-number',
        action='append', metavar='NUM',
        help='add number to filter'
    )
    aparser.add_argument('-t', '--filter-time',
        action='append', metavar=('START', 'END'), nargs=2,
        help='add time range to filter, START/END can be in seconds since epoch or YYYY-MM-DD HH:MM:SS',
    )

    aparser.add_argument('--replace-num-num', '--rnn',
        action='append', metavar=('SEARCH', 'REPLACE'), nargs=2,
        help='replace number by number',
    )
    aparser.add_argument('--replace-num-contact', '--rnc',
        action='append', metavar=('SEARCH', 'REPLACE'), nargs=2,
        help='replace number by contact',
    )
    aparser.add_argument('--replace-contact-contact', '--rcc',
        action='append', metavar=('SEARCH', 'REPLACE'), nargs=2,
        help='replace contact name by contact',
    )
    aparser.add_argument('--replace-contact-num', '--rcn',
        action='append', metavar=('SEARCH', 'REPLACE'), nargs=2,
        help='replace contact name by number',
    )

    agroup = aparser.add_mutually_exclusive_group()
    agroup.add_argument('-r', '--remove-filtered',
        action='store_true',
        help='remove entries that match the given filters'
    )
    agroup.add_argument('-k', '--keep-filtered',
        action='store_true',
        help='keep entries that match the given filters'
    )
    aparser.add_argument('--match-any',
        action='store_true',
        help='entries will be filtered if either contact/number or time filter matches instead of needing both to match',
    )
    aparser.add_argument('--partial-match',
        action='store_true',
        help='an entry with any numbers/contacts in the filter can match instead of all of them needing to match',
    )

    aparser.add_argument('--remove-no-duration',
        action='store_true',
        help='remove call entries with no call duration'
    )
    aparser.add_argument('--remove-comments',
        action='store_true',
        help='remove comments from output'
    )

    aparser.add_argument('--revert-escape',
        action='store_true',
        help='revert escaped invalid XML'
    )

    aparser.add_argument('--strip',
        action='store_true',
        help='strips non-critical attributes from entries, MAY AFFECT RESTORATION'
    )

    aparser.add_argument('-i', '--indent',
        action='store', metavar='VALUE',
        help='indent entries by VALUE spaces, or "tab"'
    )

    aparser.add_argument('-x', '--extract-media',
        action='store', metavar='FILE',
        help='extract media files to FILE archive'
    )
    aparser.add_argument('--no-write-optimized-images',
        action='store_true',
        help='do not write optimized images into the output file'
    )
    aparser.add_argument('--image-width',
        action='store', metavar='VALUE',
        help='set maximum image width'
    )
    aparser.add_argument('--image-height',
        action='store', metavar='VALUE',
        help='set maximum image height'
    )
    aparser.add_argument('--jpg-quality',
        action='store', metavar='VALUE',
        help='set jpg image quality'
    )
    aparser.add_argument('--shrink-only',
        action='store_true',
        help='only change media if the newer data is smaller'
    )
    ...
    if any(map(
        ...
        [
            argspace.filter_number,
            argspace.filter_contact,
            argspace.filter_number_expr,
            argspace.filter_contact_expr,
            argspace.filter_time,
        ],
    )):
    ...
    if argspace.remove_filtered or argspace.keep_filtered:
        mainParser.removeByFilter(
            clFilter,
            timeFilter,
            removeFiltered=argspace.remove_filtered,
            matchesAnyFilter=argspace.match_any,
            fullMatch=not argspace.partial_match,
        )
...
def optimizeImages(mainParser, argspace, clFilter, timeFilter):
    ...
    mainParser.optimizeImages(
        clfilter=clFilter,
        timefilter=timeFilter,
        maxWidth=width,
        maxHeight=height,
        jpgQuality=quality,
        onlyShrink=argspace.shrink_only,
    )
